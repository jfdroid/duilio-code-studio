#!/usr/bin/env python3
"""
REAL File Listing Test
======================
Tests the EXACT scenario from the UI screenshot:
...
async def test_real_scenario():
    ...
    print("="*60)
    print("🧪 REAL FILE LISTING TEST (UI Scenario)")
    print("="*60)
    print()
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        # Simulate Chat mode request (with workspace_path)
        payload = {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if has_bad and not has_good:
                    ...
                    result = "⚠️  PARTIAL - Mentions files but no actual list"
                else:
...
if __name__ == "__main__":
    result = asyncio.run(test_real_scenario())
    
    print("\n" + "="*60)
    print("FINAL RESULT")
    print("="*60)
    if result.get('passed'):
        print("✅ TEST PASSED - System correctly lists files")
        print("   The UI should now show actual file list instead of Python code")
    else:
        print("❌ TEST FAILED - System needs more fixes")
        print("\nIssues found:")
